...
void Tab1DeviceControl::on_pPBTTimerStart_clicked(bool checked)
{
    if(checked)
    {
        ui->pPBTTimerStart->setText("TimerStop");
        QString strValue = ui->pCBTimerValue->currentText(); // 50
        pQTimer->start(strValue.toInt());
    }
    else
    {
        ui->pPBTTimerStart->setText("TimerStart");
        pQTimer->stop();
    }
}
